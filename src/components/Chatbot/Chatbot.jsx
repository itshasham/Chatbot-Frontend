...
const Chatbot = () => {
  ...
  // Backend URL - change this to your local server
  ...
  useEffect(() => {
    console.log("🔄 Fetching products and categories from backend...");
    
    // Fetch products
    fetch(`${BACKEND_URL}/get_products`)
      .then(res => {
        console.log("✅ Products response received, status:", res.status);
        if (!res.ok) {
      ...
      .then(data => {
        console.log("📦 Products API Data received:", data);
    ...
    fetch(`${BACKEND_URL}/get_categories`)
      .then(res => {
        console.log("✅ Categories response received, status:", res.status);
        if (!res.ok) {
          throw new Error(`HTTP error! status: ${res.status}`);
        }
        return res.json();
      ...
      .then(data => {
        console.log("🏷️ Categories API Data received:", data);
        
        if (data.success && data.categories) {
          setCategories(data.categories);
          console.log("✅ Categories set in state:", data.categories.length, "categories");
        } else {
  ...
  useEffect(() => {
    console.log("🏷️ categories state updated:", categories);
  ...
  const handleCategoryClick = (categoryName) => {
    handleUserMessage(`Show me ${categoryName.toLowerCase()} products`);
  };
  ...
  const handleUserMessage = async (text) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/get_answer`, {
        method: 'POST',
        headers: {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/save_lead`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
